...
def create_data():
    train = pd.read_csv(r'C:\Users\markd\OneDrive\Рабочий стол\data\train.csv')
    print("Dimension of train data {}".format(train.shape))
    return train

def create_data_test():
    test = pd.read_csv(r'C:\Users\markd\OneDrive\Рабочий стол\data\test.csv')
    print("Dimension of test data {}".format(test.shape))
    return test

def head_tail(train, test):
    print(train.head())
    print(test.tail())

def distrib_viz(train, test):
    plt.rc('font', size=13)
